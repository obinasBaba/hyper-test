import { Divider, IconButton, Popover, Slide, SvgIcon, TextField, useScrollTrigger } from '@mui/material';
...
export const TopNav = () => {
  ...
  const handleClick = (event: React.MouseEvent<HTMLButtonElement>) => {
    setAnchorEl(document.querySelector('#top_nav_wrapper') as HTMLButtonElement);
  };
  ...
  return (
    <HideOnScroll>
      <div className={s.top_nav}>
        <div className={s.wrapper} id="top_nav_wrapper">
          <Stack alignItems="center" direction="row" gap=".5rem">
            <Logo />

            <Button startIcon={<MenuOutlined sx={{ mb: '.2rem' }} />} onClick={handleClick} className={s.menu_btn}>
              카테고리
            </Button>
          </Stack>
          ...
          <TextField
            ...
            size="small"
            InputProps={{
              startAdornment: <Search color="disabled" sx={{ mr: '.5rem' }} />,
            }}
          />
          ...
          <Stack direction="row" gap=".5rem" alignItems="center" className={s.other_btns}>
            <IconButton>
              <SvgIcon>
        ...
        >
          <div className={s.poper_content}>
            {
              // create 10 leng array and loop
              Array.from({ length: 10 }).map((_, index) => (
                <Stack key={index} alignItems="center">
                  <Image src={JoyStick} alt="joy" style={{ maxWidth: '5rem' }} />
